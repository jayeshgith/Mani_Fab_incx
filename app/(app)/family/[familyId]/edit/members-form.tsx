...
export default function EditFamilyMembersForm({
  ...
  const [savedName, setSavedName] = useState(familyName.trim());
  ...
  async function onSaveName() {
    ...
    try {
      const renameResponse = await fetch(`/api/families/${familyId}`, {
        method: "PATCH",
      ...
      if (!renameResponse.ok) {
        setMessage(renameData?.error ?? "Unable to update family name.");
        setSaving(false);
        return;
      }
      ...
      setMessage("Something went wrong while updating family name.");
      setSaving(false);
      return;
    }
  ...
  return (
    <div className="mx-auto w-full max-w-5xl px-4 py-6 sm:px-6">
      ...
      <div className="space-y-6 rounded-2xl border bg-white p-5 shadow-sm sm:p-6">
        <div>
          <p className="mb-2 text-sm font-medium text-slate-700">Family Name</p>
